      ...
      <div className="w-[87%] mx-auto min-h-screen ">
        ...
        <div className="flex flex-col gap-8 justify-center items-center mt-15">
          {faqs.map((item: FAQ) => (
            <div
              key={item.id}
              className="flex flex-col w-[750px] bg-[#FAFAFA] shadow-lg border border-black/10 border-t-0 rounded-3xl"
            >
              <button
                onClick={() => toggleFAQ(item.id)}
                className="flex items-center justify-between px-12 py-6 w-full"
              >
                <h3 className="font-semibold text-[21px]">{item.question}</h3>
              ...
              {/* Answer */}
              {openId === item.id && (
                <p className="px-12 pb-10 text-text text-[16px] leading-7">
                  {item.description}
                </p>
              )}
            </div>
